...
const { sqlFunction } = require('./database');
...
const addToTable = async (table, values) => {
	...
	case 'role':
		...
		department = await actions.getTable('department', 'id', values[2]);
	...
	case 'employee':
		...
		role = await actions.getTable('role', 'id', values[2]);
		manager = values[3] ? await actions.getTable('employee', 'id', values[3]) : [{ first_name: null }];
